...
### R79. An owed criterion whose `wake_at` passes unmet falls into a state no predicate can move (L-planner-0015, 2026-09-17 ~13:28Z)
- **Measured:** `L-spec-0010` (L-charter-0005) is merged (`05ed71942`, 2026-09-15), graded, reviewed, and carries two owed criteria — `AC9`, `AC10`, both `wake_at: 2026-09-16T12:00:00Z`, neither with an `owed-met`. It derives as bare **`shipped`**. Read `fold.py:867-882`: `accepted` needs `owed_criteria <= met_criteria`, which fails; `shipped-owed-evidence` needs an unmet `owed-ac` whose `wake_at` is **in the future**, which also fails now that the date has passed. Both branches fall through to the pipeline-state loop at `:895`, which returns `"shipped"`.
- **Why that is worse than either honest answer.** `shipped` is **not** in `_closable`'s `all_accepted` set (`accepted`, `shipped-owed-evidence`, `dropped`, `closed-unbuilt`, `closed-shipped`), so the spec blocks its charter's L2 forever. And because it is no longer `shipped-owed-evidence`, it **stops being counted by `owed_within_k`** — so the one instrument that is supposed to bound outstanding evidence silently loses track of it. An overdue obligation becomes *less* visible than a pending one, at exactly the moment it becomes more urgent.
- **Nothing emits on the transition.** The state changes because the clock moved, not because an event landed, so no board row, no tick, no notification fires. `doit states` showed `L1-complete` for a charter whose code shipped two days ago and whose only outstanding item is two un-run production observations.
- **This is the visible half of a larger reporting gap.** Charters 0005, 0006 and 0009 all read `L1-complete` while their code is on master. The other blockers are honest (`L-spec-0021` carries standing rejects on `AC2` and `done-condition`; `L-spec-0019` has four grader spawns and not one confirmed verdict; no charter-review has ever completed, so `review_owed` is True everywhere). But the operator's read — "most of those are done, something is wrong with the reporting" — is correct on substance: the board answers *"has this charter closed"* and is routinely read as *"has this work landed"*, and for three charters those two answers have been opposite for two days.
- **Also measured, and a separate trap:** `L-charter-0020` renders with **zero specs** in an `albert-scott`-scoped pane, because `read_events()` filters on `DOIT_PROJECT` and 0020's specs are `do-it-v2`. The charter row survives the filter; its specs do not. A scoped pane therefore shows a real charter as empty rather than as out-of-scope — the same `DOIT_PROJECT` hazard already recorded against `test_fold.py`, one level up.
- **Systemic:** an expired unmet `owed-ac` needs a state of its own — `owed-overdue`, counted by `owed_within_k` and rendered on the board — or `shipped-owed-evidence` should drop the future-`wake_at` condition and let the K bound do the work it exists to do. Either way the transition should be an event, not a clock reading.
- **Fix status:** open — no change-list id.
