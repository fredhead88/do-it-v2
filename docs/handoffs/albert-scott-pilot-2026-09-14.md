...
### R81. R75 is not an edge case: three of three spec-writers wrote a Verification block the packet builder refuses, and one of them reached for `alembic upgrade head` (L-planner-0016, 2026-09-17 ~14:10Z)

- **Measured, 3 of 3 on one charter's wave 1.** Every spec-writer spawn returned a schema-VALID output card, the wrapper appended `spec-written`, and all three specs were durable ledger artifacts before any check read their **body**. Then `doit packet spec-auditor` refused all three, for two distinct reasons: `L-spec-0064` and `L-spec-0066` — "Verification block is not one gated `&&` chain"; `L-spec-0065` — "names an interpreter by bare word, not an absolute path: `.venv/bin/pytest`". R75 recorded this once and called it systemic; a 100% failure rate on the next charter settles it.
- **The rule is enforced but never stated.** `packet.py` knows the Verification block must be one `&&`-gated chain with absolute interpreter paths. `agents/spec-writer.md` and the packet's own item 10 (the template) say neither thing. Three independent Sonnet spawns each invented a plausible multi-line block, because nothing told them the shape and nothing checked it at write time. `doit validate spec-writer` validates the **card**; the body is unvalidated until an auditor wants it.
- **The repair cost is real but it is not the finding.** D24 gives a spec one `spec-writer` round, and spending it on a formatting defect would leave the actual audit's findings unfixable — so, as in R75, the already-stamped sub-agents were resumed for a text edit. Cheap and correct, and **outside the ledger's spawn accounting**: `doit spend` under-counts these three specs by 17 tool uses and ~125 s across five resumes.
- **The dangerous half, and it is new.** Asked for a formatting-only fix, `L-spec-writer-0118` did not merely gate the existing three commands — it **added two**: a step piped to `true` (which cannot fail, so it is not a gate, inside a chain whose whole purpose is gating), and **`alembic -c api/alembic_supabase.ini upgrade head`**, carrying an inline comment "run from api/, against a scratch/non-prod DB only". A Verification block is run by builders and graders in environments where `SUPABASE_DB_URL` may point at production, and `deploy.sh` already runs `upgrade head` against prod on every deploy. A comment is not a safety mechanism. The unit adds no migration and needs none. It was caught only because the serving pane read the block in the hand-back — **nothing in the pipeline would have stopped it**, since `packet.py`'s checks are shape checks (`&&` chain, absolute paths) and a gated absolute-path chain containing `alembic upgrade head` passes all of them. Removed on a second resume; the two sibling specs were then warned about all three defects pre-emptively in their own fix messages.
- **Systemic, in the order worth fixing.** (1) State the Verification block's shape in `agents/spec-writer.md` and in the packet template — one gated `&&` chain, absolute interpreter paths, no inline comments, no step that cannot fail, no command that writes to any database. The writers are not being careless; they are guessing at an unstated contract. (2) Run `packet.py`'s existing body check inside `doit validate spec-writer`, so a malformed spec never reaches `spec-written` — R75 already named this and it is still the right fix. (3) Add a **deny-list** check for database-writing commands (`alembic ... upgrade`, `psql ... -c "UPDATE`, `INSERT`, `DELETE`, `DROP`) in a Verification block. The shape checks cannot catch this class, and this is the first recorded instance of a spec nearly shipping a production migration command as a verification step.
- **Fix status:** open — no change-list id. (3) is the one that would have caught the dangerous half, and nothing else in the system looks for it.

## Thinker pane, 2026-09-17 06:30Z → 15:00Z (L-thinker-0004 · reboot; reorganise the open Goal-A charters into two; disk cleanup under operator ruling)

Own section per b-12. Measured while retracting L-charter-0015–0018, filing L-charter-0022/0023,
answering two Planner escalations and clearing the orchestration box from 95% to 61%.

### R82. An operator-only event appended from a role pane is dropped silently, and the board keeps reading the old state for hours (L-thinker-0004, 2026-09-17 08:46Z → 14:35Z)
- **Measured:** `doit append charter-retracted L-charter-0015 …` from `DOIT_LEDGER_FILE=L-thinker-0004.jsonl` wrote the line and printed nothing; `fold.py`'s `EMITS` table drops `charter-retracted` / `correction` / `spec-closed` unless the actor is `operator`, and the actor is derived from the ledger filename. `doit states` read `open` for six hours and the board listed both retracted charters under PLANNER WAITING ON. Re-appending under `L-operator-local.jsonl` (quoting the in-session ruling) fixed it.
- **Systemic:** the ledger accepted an event the fold will never read — durable and false at once. `doit append` knows `EMITS` and the actor before it writes; refusing (exit 1, "charter-retracted is operator-only; append under L-operator-local.jsonl with the ruling in why=") costs one lookup.
- **Fix status:** open — no change-list id. Recorded as memory `operator-only-ledger-events` for the next pane.

### R83. `--land` audits the charter set against the newest goal, not the goal the charters cite (L-thinker-0004, 2026-09-17 ~14:20Z)
- **Measured:** `think.goal_path()` takes the newest `goal-filed` event when `--goal` is absent. Two goals are filed (L-goal-0001 the Profit v2 goal, L-goal-0002 Yitzy's). Landing L-charter-0022/0023 (`Covers: G1–G6` of L-goal-0001) built a `stage: charter-set` packet against L-goal-0002 and dispatched a plan-auditor on it. Caught only because the dispatch line named the goal; dispatcher pid killed before the seat was served, a `decision` on L-goal-0002 records the miss. Rebuilt by calling `think.check` / `coverage` / `charter_set_packet` by hand with the right goal and dispatching `plan-auditor L-goal-0001`.
- **Systemic:** the charters carry the goal (their G-ids), the goal carries its G-ids; the intersection is unambiguous. `goal_path` should resolve from Covers, and refuse when two goals are filed and no `--goal` names one — "newest" is never the right default with more than one.
- **Fix status:** open — no change-list id.

### R84. A charter reads `open` on `doit states` while a Plan, a cut and three specs already exist on it, and a Thinker rewrote it underneath the Planner (L-thinker-0004, 2026-09-17 ~14:15Z; Planner escalation at 14:40Z)
- **Measured:** L-charter-0022's Plan (`plan-L-charter-0022.md`), cut and L-spec-0064/0065/0066 landed between 09:00Z and 13:30Z. `doit states` read `open`; the board read it as throttled (no PLANNER line on it). The Thinker, reorganising the open set under an operator ruling, widened R4 (fleet-wide `SKIPPED` honesty) and moved figures without running `doit events L-charter-0022` first. The Planner's mid-flight spec-writer then saw a charter that contradicted its cut and escalated ("R4 fleet-wide vs the-job-records-its-own-end t2_seller-only"). Resolved by taking the Planner's default: R4 narrowed back; the fleet-wide half named as L-charter-0023 R20; `charter-revised` on both with `ref=` the escalation; `decision` on 0022.
- **Systemic:** (1) `open` conflates "nothing has touched it" with "a Planner is mid-cut on it" — `plan-written` / `cut-written` should surface as a state (`planning`) on `doit states` and the board. (2) A `charter-revised` on a charter with a `cut-written` event should require `ref=` (the escalation or decision it answers) or refuse — a silent rewrite under a cut is the one edit the Planner cannot see coming. (3) `agents/thinker.md`: before revising a filed charter, read its events.
- **Fix status:** open — no change-list id. (2) is the one that would have stopped this.

### R85. The mirror of R79: `open` is read as "not started", and the Thinker misbriefed the operator on boot (L-thinker-0004, 2026-09-17 ~07:00Z)
- **Measured:** first read of the board: "L-charter-0005 blocks everything; nothing is plannable". Operator: "You're 100% wrong. Most of that stuff is done and just has one or two little things". Re-measured on the deployed sha: 0005/0006/0009/0019/0020/0021 were L1-complete with one or two open ACs each; only 0015–0018 were genuinely unstarted, and those were mergeable pairwise. R79 measured closed-read-as-landed; this is open-read-as-unstarted, from the same board.
- **Systemic:** the board has no per-charter "how much of this is built" figure (accepted specs / cut specs, open ACs). One column would have made the boot read correct. Recorded as the second measurement of the R79 class.
- **Fix status:** open — folds into whatever R79 gets.

### R86. Two small leaks measured during the disk cleanup (L-thinker-0004, 2026-09-17 14:00–14:35Z)
...
### R107. A Planner told to record a correction has no event type that survives the fold (L-planner-0018, L-charter-0024, 2026-09-20)
- **Measured:** `fold.EMITS["correction"] == {"operator"}` (src/fold.py:32, D111). A Planner pane's `doit append correction …` writes to `events/L-planner-NNNN.jsonl` and **exits 0, printing the board as though it landed** — the drop happens later, inside the fold, silently. Two such events sit in this charter's ledger files and neither is applied. Nothing in `agents/planner.md` names `correction` as unavailable, and the Planner contract explicitly requires the pane to correct its own Plan's figures when a measurement contradicts them (R93's whole lesson).
- **Why it bites here:** the content being corrected was the R8 cost baseline, which is the criterion of an uncommissioned unit. A silently-dropped correction is worse than a refusal: the pane believes the record exists and stops carrying the fact.
- **Workaround used:** re-file the identical text as a `decision`, which a Planner may emit, and amend the Plan file. The Plan file is the artifact that actually reaches a spec-writer (the packet builder inlines the shared-decisions table, never the ledger), so the Plan edit is load-bearing and the event is the audit trail.
- **Systemic:** either `doit append` refuses an unauthorized type at the call site with a non-zero exit, or `EMITS` admits a planner-scoped correction. An append that exits 0 and is discarded is an action whose only record is the conversation — the one thing §9.2 exists to prevent.
- **Fix status:** open, worked around.

### R108. The v1 spec ledger could not be validated at all, and `--check` reported it as a crash rather than as a finding (L-planner-0018, L-charter-0024, 2026-09-20)
- **Measured:** `python scripts/spec_ledger.py --check` aborted with `yaml.scanner.ScannerError: mapping values are not allowed in this context` on the **first** unparseable record and never reached the other 1,183. **29 records** were affected. Two authorship paths produced them, both writing YAML by hand rather than through `spec_ledger.py set`: (a) a 2026-09-17 operator reconciliation pass that wrote `by: operator (reconciliation 2026-09-17: merge commit is an ancestor of …)` — an unquoted scalar containing `: `; (b) grader `rework_reason` text carrying check output with embedded colons and an unbalanced `'`. The hold-sweep cron was swallowing the same failure as a one-line `warning: classify failed: ScannerError(...)` on every run and continuing, so nothing escalated.
- **Why it matters:** the ledger is the project's stated answer to "did we write any specs that never got built?", and CLAUDE.md's own diagnosis discipline says *never take a spec's status from prose — read `~/.claude/ledger/<NNN>-*.yml`*. For an unknown period the instrument backing that rule was returning a stack trace, and the daily `albert-scott-ledger-check` cron was reporting a warning instead of a failure.
- **Fixed here:** all 29 repaired (backups under `/tmp/claude-1000/ledger-bak/`); `--check` now returns **`Ledger OK — 1184 record(s)`**. The repair only added quoting and re-emitted one `rework_reason` as a literal block; no status, history or field value was altered.
- **Systemic:** two separate defects. (1) `--check` should parse every record and report the bad ones as findings, not abort on the first — a validator that dies on its first input cannot measure its own corpus. (2) The hold-sweep's `classify failed` warning should be an error; it was the only surviving signal and it was tuned to be ignorable.
- **Fix status:** data repaired; both systemic defects open.

### R109. ROOT CAUSE of R81/R92, ten occurrences in: the packet builder strips shell comments without respecting quotes (L-planner-0018, L-spec-0120, 2026-09-20)
- **The symptom, nine times re-diagnosed as a formatting slip:** `doit packet spec-auditor` refuses a Verification block saying a named line *"does not end in `&&`"* — when the line, on disk, visibly **does** end in `&&`.
- **The cause:** the builder strips `#` comments from the block **without respecting quoting**. `L-spec-0120`'s block was well-formed; its two failing lines were the only two containing a literal `#` inside a quoted grep pattern:
  - `grep -q '^# ' docs/architecture/ci_cost_model.md &&` → truncated at the `#` to `grep -q '^`
  - `head -n1 … | grep -qv '^# Albert Scott' &&` → truncated to `head -n1 … | grep -qv '^`

  After truncation the parser's complaint is **correct**; the message just describes a line the author cannot see. That is why this has cost ten diagnoses and never once been fixed at the source.
- **Why it will keep happening:** any spec whose Verification block greps for a markdown heading trips it, and this repo's own convention — *"Markdown files: never start with the project name as H1"* — makes exactly that grep the natural way to verify a document unit. The class is wider than markdown: any quoted `#` (a colour literal, a comment-syntax assertion, an anchor link, `awk '$1 ~ /^#/'`) detonates identically.
- **Workaround used** (mechanical, meaning preserved, proved on positive and negative fixtures): re-express without a literal `#` — `/usr/bin/awk 'NR==1{exit !(substr($0,1,2)==sprintf("%c ",35))}' FILE` for the H1 test, and `/usr/bin/head -n1 FILE | /usr/bin/grep -qv 'Albert Scott'` for the project-name test. Backup at `/tmp/claude-1000/L-spec-0120.bak`. The D24 rework round was preserved for the audit, as with R92's earlier instances.
- **Systemic:** two fixes, both small. (1) Strip comments with a quote-aware scan — a `#` inside `'…'` or `"…"` is not a comment. (2) Make the refusal quote the line **as the parser sees it after stripping**, not as the author wrote it; a refusal that describes a state the file does not exhibit is unactionable, and nine panes have proved it.
- **Also still true and still unwritten:** the Verification-block rules (absolute interpreter, one gated `&&` chain, no trailing backslash, no bare `;`, no `||`) appear in **no contract**. This pane now ships them in every slot packet by hand.
- **Fix status:** open, root-caused, worked around.

### R110. R100 recurs in mirror image — and the lesson is not "edit the right copy", it is "there must not be two" (L-planner-0018, L-charter-0024, 2026-09-20)
- **R100 was:** an ADR amended in its own file but **not** in the Plan's Shared-decisions table; the packet inlines the table, so the writer obeyed the stale row and the audit correctly called `bad_cut`. Cost: one whole spec.
- **This time it went the other way.** Measured billing figures landed in the Plan's shared-decision rows 10 and 12, and `content/L-adr-0113.md` / `L-adr-0115.md` were **not** amended — so the ADR files still asserted a withdrawn `$0.00623/min` rate and a `~$141/month` baseline that a live API read had already disproved.
- **Caught by the spec-writer, not by the Planner.** `L-spec-writer-0193` found it in pre-flight, wrote against the Plan's table (**correct** — the packet inlines the table, never the ADR file), flagged the staleness in its own Assumptions, and then independently re-verified every figure against `GET /users/fredhead88/settings/billing/usage`, reproducing all of them exactly (4,303 min/$7.71; 17,174.67 min/$85.00; 19,534.33 min/$98.64; rate 0.006000; ~$18/mo allowance in each of three months). Cost this time: **zero specs** — because the writer measured instead of trusting.
- **The real lesson:** R100 was filed as a discipline problem ("remember to update both"). Two charters and two directions later, that reading is wrong. **A decision stored in two places will drift regardless of which one you are careful about.** The ADR file and the Shared-decisions row are the same fact with two authors and one consumer.
- **Systemic:** the Plan's table should be **generated from** the ADR files (or the ADR body inlined by reference), so the packet and the ADR cannot disagree. Until then, `doit audit plan` should diff every ADR file against its table row and report drift as a mechanical check — it is exactly the kind of thing the mechanical pre-pass exists to catch, and it would have fired here and in R100.
- **Fix status:** both files amended and re-filed; systemic defect open, and **re-filed as systemic rather than as discipline**.

### R111. Six Planner-origin defects in one charter, and every one was an inference wearing a measurement's clothes (L-planner-0018, L-charter-0024, 2026-09-20)
- **The count.** Six defects in `L-charter-0024` originated in the Planner's own Plan or slot packets. None reached a built spec; all six were caught downstream, five by a spec-writer's pre-flight or a blind spec-audit re-running the command. Listed, because the shape only becomes visible in aggregate:

  | # | The claim I wrote | What was actually true | Caught by |
  |---|---|---|---|
  | 1 | "71 non-master push runs" | 129, across 95 branches — `gh api --paginate` silently caps at 1,000 | `L-spec-auditor-0072` |
  | 2 | `$0.00623/min`, `~$141/mo` | exactly `$0.006/min`, `~$160/mo` — a NET bill divided by GROSS minutes | own re-measure, prompted by Q2 |
  | 3 | ADR table amended, ADR **files** not | two copies of one decision, drifted | `L-spec-writer-0193` pre-flight |
  | 4 | "the guard *cannot* fail a job" | it *does* block, via the job's aggregator step — fragile, not inert | `L-spec-writer-0191` pre-flight |
  | 5 | "a flat 3,000-min / $18.00 allowance" | observed $18.05–$18.56; the derived target `17,167` is falsified by August's measured 17,174.67 min = $85.00 exactly | `L-spec-auditor-0075` |
  | 6 | "the repo's pinned pytest 9.0.3" | repo pins `>=8.0.0,<9.0.0`; 9.0.3 is only what the **builder box's venv** has | `L-spec-auditor-0078` |

- **The common cause is NOT "a number written down without running the command."** That was R93's reading and it is too kind. In all six the command *was* run. The defect is the step after: **an inference layered on top of a measurement and then recorded as if it were the measurement.** A venv's pytest became "the repo's pin". A `null` `required_status_checks` became "master has no protection". A `~$18` discount became "a flat $18.00". A `continue-on-error` on one invocation became "cannot fail a job". Each is one short, plausible hop from something true — which is exactly why none of them looked like a guess when written.
- **A seventh, different in kind and worth separating:** the slot packets told every writer "every interpreter must be an absolute path" without scoping it to the **Verification block**. `L-spec-writer-0191` applied it faithfully to *runtime code on `ubuntu-latest`* and offered `/opt/albert-scott/.venv/bin/python`, which would have shipped a guard that crashes on every push — a fail-open guard, in the unit whose purpose is making a guard fail closed. All four wave-3 packets carried the unscoped wording. **An instruction followed correctly that produces a defect is a packet bug, not a writer bug**, and it is invisible to the writer by construction.
- **What actually held the line.** Not the Planner's self-review, which passed all six. The blind spec-audit and the writers' pre-flight measurement did, and they did it by **re-running the command against HEAD rather than reading the packet's prose**. Two of the six were caught by a writer *before* any audit — the pre-flight is not a formality.
- **Also worth recording: the audits were precise, not merely loud.** Across four spec-audits, 33 findings were confirmed and 34 candidates were **rejected on inspection** (including four citation-freshness checks that held, the `grep -c` exit-code concern, and this Planner's own absolute-interpreter convention, rejected twice as a defect and correctly so). A round that rejects as much as it confirms is measuring, not pattern-matching.
- **Systemic:** the mechanical pre-pass already diffs requirement ids and seams. It should also diff **every ADR file against its Plan table row** (would have caught #3, and R100 before it), and the Plan's stated measurements should carry the command that produced them so a reader can re-run one line instead of re-deriving the claim (#1, #2, #5, #6 are all re-runnable in seconds).
- **Fix status:** all six corrected at source, in the Plan *and* the ADR files in the same pass; the interpreter-scoping correction issued to the affected rework. Systemic items open.

### R112. An in-scope brief holds a charter open and nothing consumes it: `after:` on that charter is then a deadlock (L-thinker-0004, 2026-09-20)
- **Measured:** six `brief{requirement}` events on L-charter-0021 (2026-09-19) hold its sweep open; L-charter-0025 is filed `after: L-charter-0021`. The launcher (`up.py`, alive 2.5 days) ran L-planner-0017 (0023) and L-planner-0018 (0024, l1-complete 2026-09-20 05:28Z), then printed `PLANNER WAITING ON L-charter-0025 · after: L-charter-0021 has not landed` and idles. `relay.py` and `up.py` contain no reference to briefs: an L1-complete charter is never plannable again, so no Planner is started for its briefs. The one precedent (L-spec-0063 under L-charter-0005) was commissioned by a human prompting a Planner pane, not by the loop.
- **Systemic:** `open_briefs` has a holder (the sweep) and no consumer. A charter with open in-scope briefs and no unwritten spec answering them should be plannable for a *corrective pass* (brief → slot → spec, D24 as usual), and the board should say "N briefs await a corrective cut", not only "after: … has not landed".
- **Fix status:** open — no change-list id. Until then a human points a Planner at the briefs.
