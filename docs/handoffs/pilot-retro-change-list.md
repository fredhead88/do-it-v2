...
| 38 | **`SEAT PENDING (n)` is a standing board block.** Every seat spawn with `spawn-started`, no terminal event and no `seat/<spawn>.output.json`, oldest first with its age and dispatcher — the fold's cross-pane serving trigger (a-20's alarm as a board line, not a threshold), so a pane that serves foreign packets sees them without reading the directory. | R50, R51 | two Planner spawns expired unserved at 0 tokens (15 + 15 min of stage clock) while the serving pane stamped only its own; confirmed symmetric — the serving pane's own 4-hour lapse cost the same charter a second time, caught only by a manual `ls` | `src/fold.py` (board), `scripts/seat/README.md` |
| 39 | **`p_spec_writer`'s fix-list only includes an `audit-finding` with no LATER `spec-written` on the subject** (or spec-auditor gains a `criterion-cleared`-style event a rework can emit). Without it, every rework packet after round 2 replays round 1's already-fixed findings forever. | R52 | a third-round rework packet would have handed a spec-writer a fix list every item of which was already applied two rounds ago | `src/packet.py` |
| 39 | **The pre-pass sees a cross-branch import and a top-heavy wave 1.** (a) A symbol defined in one unit's new-file footprint and named in a same-wave sibling's Goal or `Consumes:` is flagged as same-wave overlap; (b) a wave 1 carrying more than half the cut's units or footprint files is flagged against §3.7's "accumulated extracts, small". | R51 | one Opus round (and 4h42m of serving delay) on a cut two string checks would have refused | `src/audit.py` |
| 40 | **The sibling-body strip ignores the packet's own inputs, and a refusal is not a path.** `packet.py::sibling_bodies` drops any long line that also appears in the cut, the Plan, the charter or the builder's template before checking the packet; `doit packet` exits non-zero on refusal with the message on stderr and nothing on stdout. | R54 | every wave-2 packet of a charter refused once its first spec quoted a seam; one dispatch launched with the refusal text as its packet | `src/packet.py` |
...
| 27 | **A charter with more waves than the one-wave-ahead rule allows does not end a Planner context short of L1.** Either the fold derives `l1-complete` when every slot the rule permits is `written` and the held slots are named on the event, or ⑥ reads "one wave ahead of the wave in build, two when the later wave consumes only merged seams". | R55 | L-charter-0006 left its first Planner context at 8/9 slots with no L1 by rule, not budget | `agents/planner.md` ⑥/⑦, `src/fold.py` |
