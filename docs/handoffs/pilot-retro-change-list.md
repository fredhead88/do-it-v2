...
| 20 | **`probe.md` requires the read-only proof in the run record** when the credential it holds has write scope: the session-level setting named and its `SHOW` output pasted once per session; a root shell's command list captured verbatim. | R28 | `agents/probe.md` |
