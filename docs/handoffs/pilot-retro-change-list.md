...
| 24 | **Owed questions and named-for-veto rulings are events**: `escalation-owed{charter, q, default, revert}` on the Planner's EMITS, a board block `OWED QUESTIONS (n)` with defaults, answered by an operator `decision` whose `ref` is the question's `src`; the Plan cites the events. | R46 | nine owed questions and six rulings invisible to the board; operator asked in-pane | `design/system-design-v2.md` §8.7, `agents/planner.md`, `src/fold.py` |
