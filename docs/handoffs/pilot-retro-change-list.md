...
| 23 | **Seat spawns have a server.** `agents/planner.md`: the Planner pane serves every pending seat packet under `seat/` whoever dispatched it (`doit dispatch --seat` lists them; a-20's `seat-unserved` alarm is the trigger). `agents/thinker.md`: the Thinker may serve the one audit its own `--land` produced and no other spawn — D73 restated for the seat backend. | R35 | a charter-set audit sat unserved between two panes until the operator asked | `agents/planner.md`, `agents/thinker.md`, `design/system-design-v2.md` §3.3 |
